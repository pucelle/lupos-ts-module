...
export function createTypeHelpers(ts: typeof TS, typeCheckerGetter: () => TS.TypeChecker, core: HelperCore, context: HelperGroupContext) {
	const {isVariableIdentifier, isFunctionLike, isPropertyLike, isPropertyOrGetAccessor} = core
	const {access, symbol} = context
	const theAssignableChecker = assignableChecker(ts, typeCheckerGetter)
