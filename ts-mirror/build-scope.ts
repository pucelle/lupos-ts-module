...
interface ComponentUse {
	...
	/** Template element owning the component instance. */
	node: HTMLNode

	/** Static component name or dynamic placeholder. */
	tagName: string

	/** Resolved component declaration and metadata. */
	component: LuposComponent

	/** Generated instance identifier shared by the element parts. */
	instanceName: string

	/** Constructor expression for a dynamic component. */
	constructorValue?: TS.Expression
}


/** Operations supported by component constructor name anchors. */
...
export function buildScope(parts: TemplatePart[], template: TemplateBasis, analyzer: Analyzer, createIdentifier: () => string) {
	...
	function emitEvents() {
		for (let part of parts) {
			...
			let beComponentEvent = !!component && (
				part.namePrefix === '@@'
